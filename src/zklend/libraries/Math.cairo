...
func Math_shl{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(a : felt, b : felt) -> (res : felt):
    alloc_locals
    ...
    if le_250 == 1:
        let (shift) = pow(2, b)
        let (shift_u256) = SafeCast_felt_to_uint256(shift)
        let (a_u256) = SafeCast_felt_to_uint256(a)
        ...
        let (res) = SafeCast_uint256_to_felt_unchecked(Uint256(product_low.low, trimmed_high))
        return (res=res)
    else:
        return (res=0)
    end
